...
import { GitBranch, Mail, Lock, AlertCircle, CheckCircle2, ArrowLeft } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      if (data.session) { router.push('/projects'); router.refresh(); return }
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setMessage('Un lien de réinitialisation a été envoyé à votre adresse email. Vérifiez également vos spams.')
  }
  ...
  /* ── Titre selon le mode ── */
  const title = mode === 'register' ? 'Créer un compte'
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-white to-teal-50 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          {/* En-tête avec bouton retour si mode forgot */}
          ...
          {mode === 'forgot' ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="email-forgot" className="text-slate-700 font-medium">Adresse email</Label>
            ...
            <>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="password" className="text-slate-700 font-medium">
                      Mot de passe
                      {mode === 'register' && <span className="text-slate-400 font-normal text-xs ml-1">(6 caractères min.)</span>}
                    </Label>
                    {mode === 'login' && (
                      <button
                        type="button"
              ...
              <p className="text-center text-sm text-slate-500 mt-6">
                ...
                <button
                  type="button"
                  onClick={() => resetState(mode === 'register' ? 'login' : 'register')}
